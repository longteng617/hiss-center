...
/**
 * Task_AddMultiInstanceExecutionHandler
 *
 * @author: wgl
 * @describe: 后加签任务处理器
 ...
 */
@Component
public class Task_AfterSignTaskHandler implements CmdHandler<AddExecutionTaskMessage> {
    ...
    public void invoke(Message params, MessageContext messageContext) {
        ...
        try {
            ...
            if (multiInstanceVo.getType() instanceof ParallelMultiInstanceBehavior) {
                ...
                messageContext.setResult(new ConcurrentHashMap<>() {
                    {
                        put("msg", "并行多实例节点不允许加签");
                    }
                });
                return;
            } else if (multiInstanceVo.getType() instanceof SequentialMultiInstanceBehavior) {
                //如果是串行多实例节点，则需要先完成当前任务，再加签
                AddSequenceMultiInstanceCmd addSequenceMultiInstanceCmd = new AddSequenceMultiInstanceCmd(task.getId(), runtimeService.getVariables(task.getExecutionId()), addExecutionTask.getUserId(), addExecutionTask.getUserName(), taskService,false);
    ...
    @Override
    public String getId() {
        return HandlerIdClientEnum.TASK_ADDEXECUTION_TASK.getId();
    }
}
