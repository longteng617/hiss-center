package cn.itcast.hiss.process.activiti.handler.client.process.instance;
...
@Component
@Slf4j
public class CLI_ProcessInstanceDeleteHandler implements CmdHandler<ProcessInstanceMessage> {
    @Autowired
    ...
    public void invoke(Message params, MessageContext messageContext) {
        ...
        if(!canQuery){
            List<String> userAppIdsList = hissUserAppMapper.getUserAppIdsList(palyload.getAdminId());
            canQuery = userAppIdsList.contains(palyload.getTenantId());
        }
        if(canQuery) {
            processInstanceService.deleteProcessInstance((ProcessInstanceMessage) params,messageContext);
        }else{
    ...
    @Override
    public String getId() {
        return HandlerIdClientEnum.CLI_PROCESS_INSTANCE_DELETE.getId();
    }
}
