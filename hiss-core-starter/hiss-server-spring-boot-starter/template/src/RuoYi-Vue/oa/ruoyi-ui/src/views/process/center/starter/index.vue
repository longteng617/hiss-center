...
import {getUserApplayProcessList, delUserApplayProcess} from '@/api/process'
  ...
  methods: {
    handleDelete(row) {
      this.$modal.confirm('是否确认删除名称为"' + row.name + '"的数据？')
        .then(async () => {
          console.log(row)
          const res = await delUserApplayProcess(row.id)
          if (res.code === 200) {
    ...
    async getList() {
      this.loading = false
      try {
        const res = await getUserApplayProcessList(this.searchData) // 获取列表数据
